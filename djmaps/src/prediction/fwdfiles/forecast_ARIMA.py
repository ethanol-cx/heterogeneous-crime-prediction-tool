...
def forecast_timeseries_ARIMA(timeseries, forecasted_data, cluster_cntr, cluster, periodsAhead_list, gridshape, ignoreFirst, threshold, maxDist):
    pred_model = None
    ...
    params = getIfParametersExists(
        "ARIMA", gridshape, cluster, ignoreFirst, threshold, maxDist)
    if params:
        print("Loading existing parameters for ...")
        pred_model = sm.tsa.statespace.SARIMAX(endog=timeseries, order=params[0], seasonal_order=params[1],
        ...
        if stepwise_model.seasonal_order:
            pred_model = sm.tsa.statespace.SARIMAX(
                endog=timeseries, order=stepwise_model.order, seasonal_order=stepwise_model.seasonal_order, enforce_stationarity=False, enforce_invertibility=False, hamilton_representation=False)
        else:
            pred_model = sm.tsa.statespace.SARIMAX(
                endog=timeseries, order=stepwise_model.order, enforce_stationarity=False, enforce_invertibility=False, hamilton_representation=False)
    ...
    for periodsAhead in periodsAhead_list:
        print("ARIMA", threshold, cluster, periodsAhead)
        periodsAhead_cntr += 1
...
def forecast_ARIMA(method, clusters, realCrimes, periodsAhead_list, gridshape, ignoreFirst, threshold, maxDist, orders=[], seasonal_orders=[]):
    ...
    for c in clusters.Cluster.values:
        ...
        forecast_timeseries_ARIMA(df, forecasted_data, cluster_cntr, c,
