...
import os
from .general_functions import savePredictions
...
def forecast_timeseries_LSTM(model, timeseries, forecasted_data, look_back, batch_size, cluster_cntr, cluster, periodsAhead_list, gridshape, ignoreFirst, threshold, maxDist):
    ...
    df = scaler.fit_transform(np.array(timeseries).reshape(-1, 1))
    ...
    for periodsAhead in periodsAhead_list:
        print('LSTM, threshold: {}, cluster: {}, periodsAhead: {}'.format(
            threshold, cluster, periodsAhead))
        periodsAhead_cntr += 1
...
def forecast_LSTM(clusters, realCrimes, periodsAhead_list, gridshape, ignoreFirst, threshold, maxDist):
    ...
    for c in clusters.Cluster.values:
        ...
        forecast_timeseries_LSTM(model, df, forecasted_data, look_back, batch_size,
                                 cluster_cntr, c, periodsAhead_list, gridshape, ignoreFirst, threshold, maxDist)
