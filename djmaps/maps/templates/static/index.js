...
let dataCrimes;
let dbPredict = [];
let clusterOfEachPoint = []; //lstm cluster of each point
let timeseriesToPredict = [];

$.getJSON("./static/dataCrime1.json", function (dC) {
	dataCrimes = dC;
	let filterPoints = dataCrimes;
	...
	$("#timeOfDay").append("<in	put class = 'change' type = 'checkbox' id='tod-ALL' name='tod-ALL' font='sans-serif' checked></input>");
	...
	$("#LSTMsubmit").click(function () {
		...
		for (let i = 0; i < 10000; i++) {
			...
			let mapLayer = map.getLayer(currClusterID);
		...
		const url = 'http://localhost:8000/crimePred/cluster/dps/' + grid_x + ',' + grid_y + '/' + threshold;
		console.log("URL:  " + url);
		let allFilters = {
			crimeType,
			dateCommitted,
		...
		if (method == "GET") {
			$.ajax({
				...
				success: function (data) {
					...
					for (let i = 0; i < clusterBoundaries[0].length; i++) {
						let currCluster = new Array();
						let clusterID = "cluster" + i;
						for (let j = 0; j < clusterBoundaries[0][i].length; j++) {
							currCluster.push([clusterBoundaries[0][i][j][0], clusterBoundaries[0][i][j][1], clusterBoundaries[0][i][j][2], clusterBoundaries[0][i][j][3]]);
						}
						let firstPoint = [currCluster[0][0], currCluster[0][1]];
						//console.log(firstPoint);
						let orderedCluster = new Array();
			...
			$.ajax({
				...
				success: function (data) {
									...
									map.addLayer({
										...
										"source": {
											...
											"data": {
												...
												"geometry": {
													"type": "Polygon",
													"coordinates": [data[0][i].slice(path_idx.get(hash), j)]
												}
											}
										},
										"layout": {},
					...
					/*for(let j = 0; j < clustCopy.length; j++) {
						console.log("cluster boundaries at j: " + clustCopy[j]);
						for(let k = 0; k < clustCopy[j].length; k++) {
					...
					// let pointsInEachCluster = LSTMpointsInEachCluster(clustCopy, x);
					...
					// for (let y in dataCrimes.features) {
					...
					// let numZeroes = 0;
					// let numOnes = 0;
					// for (let y = 0; y < clusterOfEachPoint.length; y++) {
